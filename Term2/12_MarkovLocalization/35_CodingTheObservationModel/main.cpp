...
float observation_model(std::vector<float> landmark_positions, std::vector<float> observations, 
    ...
    for (int i = 0; i < observations.size(); ++i) {
        float psuedo_dist = std::numeric_limits<const float>::infinity();
        //float psuedo_dist = distance_max;
        if (pseudo_ranges.size() > 0) {
            psuedo_dist = pseudo_ranges[0];
            pseudo_ranges.erase(pseudo_ranges.begin());
        }
        distance_prob *= Helpers::normpdf(observations[i], psuedo_dist, observation_stdev);
    }
